/*  Group 10 - Andrew Burton, Trevor Bachand, William Peyton, & Kyra Squier
    EENG350B - SEED Lab
    Rotational Position Controller Arduino Code
    05 March 2021
    Description:
    The following program runs the rotational position feedback control algorithm. 
    A positional set point is stored as a variable, and the motor outputs are calculated to 
    get the robot to that position. 
*/
...
float phi_set = PI / 2;             //  Variable storing desired position in radians
...
void phi_controller() {
  ...
  // Output bounding
  if (phi_dot_set > 10.0) phi_dot_set = 10.0;
  if (phi_dot_set < -10.0) phi_dot_set = -10.0;
